...
HaoMD 支持为编辑器和 AI 会话窗口设置**自定义背景**（包括本地图片），打造沉浸式写作体验：

![自定义背景预览](docs/images/custom-background.png)

- 🎨 **多主题与深色模式**：支持多种主题风格，默认护眼深色模式
- 📱 **响应式设计**：适应不同窗口大小
- ⌨️ **快捷键**：完整的键盘快捷键支持
...
HaoMD 当前内置了 `File -> Export -> Word (.docx)`。

Word 导出当前特性包括：

- 可编辑的标题、段落、列表、表格、代码块和链接
- 本地图片嵌入
- Mermaid 图表导出为 PNG
- 思维导图导出为 PNG
- 导出时自动按页面宽度缩放过宽图片
...
- Mermaid / 思维导图在 Word 中以 PNG 插入，正文内容保持可编辑，但图表本身不是可编辑的 Word 图形
- 远程图片不在当前 Word 导出支持范围内，建议先使用本地图片
- 若导出结果与预期不一致，建议优先使用 `npm run tauri:dev` 验证当前本地代码，而不是旧安装包
