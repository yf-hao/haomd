# 阶段 3：可视化（Mermaid / PlantUML / XMind / KaTeX）实施计划

## 目标
- 在现有渲染管线上，完善可视化能力：Mermaid 前端渲染可配置；PlantUML/XMind 通过 Tauri 后端离线渲染；KaTeX 保持稳定。
...
1. **配置与常量落地**
   - 新建/完善 `src/config/renderers.ts`：
     ...
     - PlantUML/XMind 后端命令名、超时、输入大小/类型限制；
...
3. **后端实现（离线渲染）**
   ...
   - XMind：
     ...
     - 方案 B：解析 xmind JSON AST，生成 SVG（可选第三方库）。
...
7. **测试与验收**
   - Seed 覆盖：Mermaid、KaTeX、PlantUML、XMind 示例（含无效 UML/无效 XMind 的错误提示验证）。
   - 冒烟流程：
     ```bash
     npm run build
     npm run tauri:dev
   ...
   - 验收点：
     - Mermaid/KaTeX 正常渲染；
     - PlantUML/XMind 在后端开启时返回可视化；关闭或出错时显示占位并提示；
...
- **降级策略细化**：PlantUML/XMind 后端不可用时的 UX 规范（占位文案 + 重试提示/按钮预留），保证不出现空白区域。
