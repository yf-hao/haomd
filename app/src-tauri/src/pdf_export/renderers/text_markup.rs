...
fn parse_rgb(color: &str) -> [f32; 3] {
    ...
    let parse_channel = |slice: &str| u8::from_str_radix(slice, 16).ok().map(|value| value as f32 / 255.0);
...
fn draw_squiggly(bounds: PdfBounds, operations: &mut Vec<Operation>) {
    ...
    operations.push(Operation::new(
        "m",
        vec![Object::Real(current_x as f32), Object::Real(baseline as f32)],
    ));
    while current_x < bounds.right {
        let next_x = (current_x + step).min(bounds.right);
        let next_y = if up { baseline + amplitude } else { baseline - amplitude };
        operations.push(Operation::new(
            "l",
