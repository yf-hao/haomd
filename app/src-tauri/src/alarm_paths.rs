...
pub fn alarm_root_dir(app: &AppHandle) -> std::io::Result<PathBuf> {
    Ok(haomd_data_root_dir(app)?.join("alarm"))
}

pub fn legacy_alarm_root_dir(app: &AppHandle) -> std::io::Result<PathBuf> {
    let mut dir = app
        .path()
        .app_data_dir()
...
pub async fn ensure_alarm_root_dir(app: &AppHandle) -> std::io::Result<PathBuf> {
    let root = alarm_root_dir(app)?;
    fs::create_dir_all(&root).await?;
    migrate_legacy_alarm_root(app, &root).await?;
    Ok(root)
}

async fn migrate_legacy_alarm_root(app: &AppHandle, target_root: &PathBuf) -> std::io::Result<()> {
    let legacy_root = legacy_alarm_root_dir(app)?;
