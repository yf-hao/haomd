...
fn ensure_worker() -> Sender<AudioCommand> {
    let mut guard = AUDIO_WORKER
        .lock()
        .expect("pomodoro audio worker mutex poisoned");
                ...
                AudioCommand::Play { sound_path } => {
                    ...
                    match OutputStream::try_default() {
                        Ok((next_stream, handle)) => match Sink::try_new(&handle) {
                            ...
                            Err(err) => {
                                eprintln!("[pomodoro] create sink failed: {err}");
                                continue;
                            }
                        },
                        Err(err) => {
                            eprintln!("[pomodoro] create output stream failed: {err}");
                            continue;
                        }
                    }
                        ...
                        if let Some(path) = sound_path.as_deref() {
                            match File::open(path) {
                                Ok(file) => match Decoder::new(BufReader::new(file)) {
                                    ...
                                    Err(err) => {
                                        eprintln!("[pomodoro] decode alarm sound failed: {err}");
                                    }
                                },
                                Err(err) => {
                                    eprintln!("[pomodoro] open alarm sound failed: {err}");
                                }
                            }
                        }
...
#[tauri::command]
pub async fn play_pomodoro_alarm(
    app: AppHandle,
    ...
    let sound_path = match alarm_sound_file.as_deref() {
        Some(file_name) => match ensure_pomodoro_alarm_sound_available(&app, file_name).await {
            Ok(path) => Some(path.to_string_lossy().to_string()),
            Err(err) => {
                return err_payload(
                    ErrorCode::IoError,
                    format!("获取番茄闹钟音频路径失败: {err}"),
                    trace,
                )
            }
        },
...
#[tauri::command]
pub async fn stop_pomodoro_alarm() -> ResultPayload<()> {
    let trace = new_trace_id();
