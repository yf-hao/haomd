export function buildDocumentToolCatalogPrompt(): string {
  return (
    '\n\n当前可用文档保存/导出工具。\n' +
    '当用户明确要求“去除空行”“删除当前文档空行”“清除当前文档中的空行”时，应使用 remove_blank_lines_current_document，默认 scope=all。只删除表格与代码块之间空段落时使用 scope=table_code_gap。该工具只修改当前活动文档，不接受文件路径。\n' +
    ...
    '当用户明确要求在当前工作区内按名称删除文件或文件夹时，应使用 delete_workspace_entry。删除必须先确认，不能直接删除。\n' +
    '如果当前选择的不是该目标文件夹，或用户没有明确给出父路径，例如“删除 demo 文件夹”“删除 hello.md”，也必须先调用 delete_workspace_entry 做名称解析，不能先回复文字确认。\n' +
    '只有 delete_workspace_entry 返回唯一目标后，才允许继续进入确认删除；如果存在歧义或未找到，应直接返回解析结果。\n' +
    ...
    '如果用户明确要求在当前工作区内按名称操作，delete_workspace_entry 和 rename_workspace_entry 的 targetPath 只填写工作区内名称或相对路径，例如“temp/hello.md”“temp/demo”“demo”“hello.md”。\n' +
    ...
    '- “删除 demo 文件夹” -> delete_workspace_entry, targetPath=demo, targetKind=dir\n' +
    '- “删除 hello.md” -> delete_workspace_entry, targetPath=hello.md, targetKind=file\n' +
